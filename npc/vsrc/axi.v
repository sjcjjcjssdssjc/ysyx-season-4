...
module ysyx_22040127_axi_rw # (
    ...
    r_state == R_DATA_ADDR ? d_req_addr & 64'hffffffff_fffffff0 :
    0;
    ...
    assign d_res_valid = (r_state == R_DATA_DONE) ? // &data_read
    1'b1 : w_state == W_DONE ? 1'b1 : 1'b0;
    ...
    always @(posedge clk)begin
        if(rst)begin
            ...
            if(axi_r_last_i)begin
                ...
                d_mrdata[127:64] <= axi_r_data_i;
            end else begin
                i_mrdata[63:0]   <= axi_r_data_i;
                d_mrdata[63:0]   <= axi_r_data_i;
            end
            // if(d_req_addr == 64'h8001dc10 || d_req_addr == 64'h8001dc18)
            //     if(d_req_wen)$display("write axi with addr 0x%0h, data is 0x%0h", d_req_addr, d_mrdata);
            //     else $display("read axi with addr 0x%0h, data is 0x%0h", d_req_addr, d_mrdata);
        end
    end
    ...
    always @(posedge clk)begin
        ...
        if(rst)begin
            ...
            if(w_valid)begin
                case(w_state)
                    W_IDLE: w_state <= W_ADDR;
                    W_ADDR: if(aw_hs) w_state  <= W_WRITE;
                    W_WRITE:if(w_done_first)w_state  <= W_WRITE_WAIT;
                    W_WRITE_WAIT:if(w_done_last)w_state <= W_RESP;//burst is over
                    W_RESP: if(b_hs)  w_state  <= W_DONE;
        ...
        if(rst)begin
            ...
            if(r_valid)begin
                case(r_state)
                    ...
                    R_INST_READ: if(r_done)  r_state <= R_INST_DONE;
                    R_DATA_ADDR: if(ar_hs)   r_state <= R_DATA_READ;
                    R_DATA_READ: if(r_done)  r_state <= R_DATA_DONE;
                    R_INST_DONE: begin
                        //if(data_read)r_state <= R_DATA_READ;
                        r_state <= R_IDLE;
                    end
                    R_DATA_DONE: r_state <= R_IDLE;
    ...
    assign axi_aw_len_o     = axi_wlen;
    assign axi_aw_size_o    = axi_size;//8byte per beat
    ...
    assign axi_w_last_o     = w_state == W_WRITE_WAIT ? 1'b1 : 1'b0;
